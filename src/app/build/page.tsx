...
export default async function page(props: {searchParams:Promise<SearchParams>}) {
  ...
  const wheelTexturesURLs = wheels.map((texture) => asImageSrc(texture.texture)).filter((url): url is string => Boolean(url));
  const deckTexturesURLs = decks.map((texture) => asImageSrc(texture.texture)).filter((url): url is string => Boolean(url));
      ...
      >
        <div className="relative aspect-square shrink-0 bg-[#3a414a] lg:aspect-auto lg:grow" >
          <div className="absolute inset-0">
            <Preview
              deckTexturesURLs={deckTexturesURLs}
              wheelTexturesURLs={wheelTexturesURLs}
            />
          </div>
