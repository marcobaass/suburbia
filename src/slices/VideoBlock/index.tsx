...
const VideoBlock: FC<VideoBlockProps> = ({ slice }) => {
  return (
    ...
    <div className="relative aspect-video">
      {/* Masks */}
      <div className={clsx(
        MASK_CLASSES, "bg-brand-lime absolute inset-0 ~translate-x-2/3 ~translate-y-2/3"
      )}>
      </div>
      <div className={clsx(
        MASK_CLASSES, "bg-white absolute inset-0 ~translate-x-1/3 ~translate-y-1/2"
      )}>
      </div>
      <div className={clsx(
        MASK_CLASSES, "bg-white absolute inset-0 ~translate-x-1/2 ~-translate-y-1/3"
      )}>
      </div>
